...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		#${NAV_ID}[data-material="liquid"] .ol-preview {
			...
			/* Rim: the single neutral ring — exactly frost's treatment.
			   Colored rim shadows were tried and rejected: sub-pixel-offset
			   fringes rasterize as detached colored line segments beside the
			   ring (a second outline with a hairline gap). The chromatic
			   character lives in the background layers instead. */
			box-shadow: 0 0 0 1px var(--dsw-alias-border-l2, rgba(0, 0, 0, .1)),
				var(--dsw-shadow-lv2, 0 0 1px rgba(0, 0, 0, .2), 0 8px 24px rgba(0, 0, 0, .08));
		}
		...
		body[data-ds-dark-theme] #${NAV_ID}[data-material="liquid"] .ol-preview {
			/* One line + a soft drop — same rim discipline as light: the old
			   0 0 1px contact halo and the offset color fringes are both gone,
			   each rasterized as a hidden second outline in dark. */
			box-shadow: 0 0 0 1px var(--dsw-alias-border-l2, rgba(255, 255, 255, .14)),
				0 8px 24px rgba(0, 0, 0, .32);
		}
