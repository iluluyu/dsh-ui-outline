...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* ===== per-turn preview card =====================================
		   ...
		   presses the backdrop darker and lights from the prompt text-start
		   with a radial sheen and contained glass rim — pure CSS, same in
		   every browser, no injected elements. The rail itself stays bare
		   marks in every mode. */
		...
		#${NAV_ID} { --ol-preview-h: 106px; }
		#${NAV_ID} .ol-preview {
			position: absolute;
			/* Measured in JS: the hovered mark's center, clamped to keep the
			   card inside the rail's box (official clamp, numeric form). */
			top: var(--ol-preview-top, 0px);
			...
			transition: top .14s cubic-bezier(.2, .8, .2, 1);
		}
		...
		/* frost: token tint over the page, blur+saturate behind. */
		#${NAV_ID}[data-material="frost"] .ol-preview {
			background: color-mix(in srgb, var(--dsw-alias-bg-layer-1, #ffffff) 78%, transparent);
			-webkit-backdrop-filter: blur(14px) saturate(1.4);
			backdrop-filter: blur(14px) saturate(1.4);
		}
		body[data-ds-dark-theme] #${NAV_ID}[data-material="frost"] .ol-preview {
			background: color-mix(in srgb, var(--dsw-alias-bg-layer-1, #232324) 66%, transparent);
		}
		/* liquid: frost's readable token base, then a darker filtered backdrop
		   and a *radial* source from the prompt's text-start (top-left). This
		   removes the old washed-grey, top-to-bottom sheen: the page seen
		   through the glass is pressed down for contrast, while the highlight
		   adapts to light/dark host tokens instead of hard-coded direction.
		   It remains pure CSS — no SVG/filter host can affect page layout. */
		#${NAV_ID}[data-material="liquid"] .ol-preview {
			background:
				radial-gradient(150px 96px at 28px 18px,
					rgba(255, 255, 255, .54) 0%,
					rgba(255, 255, 255, .16) 38%,
					transparent 72%),
				/* A neutral, background-aware light plane — host brand tokens are
				   often black here, so tinting with them is what made liquid grey. */
				linear-gradient(132deg,
					rgba(255, 255, 255, .18) 0%,
					transparent 56%),
				color-mix(in srgb, var(--dsw-alias-bg-layer-1, #ffffff) 78%, transparent);
			-webkit-backdrop-filter: blur(11px) saturate(1.24) brightness(.86) contrast(1.03);
			backdrop-filter: blur(11px) saturate(1.24) brightness(.86) contrast(1.03);
			/* Start from frost's border, then add only a contained glass rim. */
			border-color: var(--dsw-alias-border-l2, rgba(0, 0, 0, .1));
			box-shadow: var(--dsw-shadow-lv2, 0 0 1px rgba(0, 0, 0, .2), 0 8px 24px rgba(0, 0, 0, .08)),
				inset 0 1px 0 rgba(255, 255, 255, .52),
				inset 0 -1px 0 color-mix(in srgb, var(--dsw-alias-label-primary, #0f1115) 8%, transparent);
		}
		body[data-ds-dark-theme] #${NAV_ID}[data-material="liquid"] .ol-preview {
			background:
				radial-gradient(150px 96px at 28px 18px,
					...
					transparent 72%),
				linear-gradient(132deg,
					color-mix(in srgb, var(--dsw-alias-label-primary, #f9fafb) 10%, transparent) 0%,
					transparent 56%),
				color-mix(in srgb, var(--dsw-alias-bg-layer-1, #232324) 66%, transparent);
			-webkit-backdrop-filter: blur(11px) saturate(1.22) brightness(.74) contrast(1.06);
			backdrop-filter: blur(11px) saturate(1.22) brightness(.74) contrast(1.06);
			border-color: var(--dsw-alias-border-l2, rgba(255, 255, 255, .14));
			box-shadow: 0 0 1px rgba(0, 0, 0, .6), 0 8px 24px rgba(0, 0, 0, .32),
				inset 0 1px 0 rgba(255, 255, 255, .22),
				inset 0 -1px 0 color-mix(in srgb, var(--dsw-alias-label-primary, #f9fafb) 12%, transparent);
		}
		...
		function findParts() {
			...
			for (const el of host.querySelectorAll("[data-chat-flow-kind]")) {
				...
				if (kind === "user") {
					...
					if (text !== "") turns[turns.length - 1].response = text;
				}
			}
		...
		function OutlineNav() {
			...
			useLayoutEffect(() => {
				...
				const btn = marks.querySelectorAll(".ol-markBtn")[near];
				if (btn === undefined) { clear(); return; }
				const rootR = root.getBoundingClientRect();
				const btnR = btn.getBoundingClientRect();
				const center = btnR.top + btnR.height / 2 - rootR.top;
				const H = 106; // --ol-preview-h
				setPreviewTop(Math.max(0, Math.min(center - H / 2, rootR.height - H)));
			...
			const rowAt = (rail, clientY) => {
				...
				const buttons = rail.querySelectorAll(".ol-markBtn");
			...
			const onMarksPointerMove = (e) => {
				...
				if (!draggedRef.current) {
					if (Math.hypot(e.clientX - start.x, e.clientY - start.y) < DRAG_START_DISTANCE) return;
			...
			const pos = side === "right"
				? { right: Math.max(0, window.innerWidth - rect.right) + 12 }
				: { left: rect.left + 12 };
			...
			return h("div", {
				...
				h("div", {
					...
					onPointerLeave: () => { if (!scrubbingRef.current) setNear(-1); },
					onBlur: (e) => {
					...
					onScroll: () => {
						...
						const btnR = btn.getBoundingClientRect();
						const center = btnR.top + btnR.height / 2 - rootR.top;
						setPreviewTop(Math.max(0, Math.min(center - 53, rootR.height - 106)));
					},
				},
					turns.map((t, i) => h(Row, {
						key: t.key,
