...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		#${NAV_ID}[data-material="liquid"] .ol-preview {
			...
			/* Rim: the base real border — one flush line, no shadow rims.
			   ...
			   ring; the chromatic character lives in the filter/light layers. */
		}
		body[data-ds-dark-theme] #${NAV_ID}[data-material="liquid"] .ol-preview {
			...
			-webkit-backdrop-filter: blur(var(--ol-blur, 12px)) saturate(1.5) brightness(.85) contrast(1.05);
			backdrop-filter: blur(var(--ol-blur, 12px)) saturate(1.5) brightness(.85) contrast(1.05);
		}
		body[data-ds-dark-theme] #${NAV_ID}[data-material="liquid"] .ol-preview {
		...
		const FX_W = 300, FX_H = 106, FX_R = 18, FX_RIM = 11;
		/* Base rim displacement (px) — reference implementations run 18–64
		   for card-sized lenses. */
		const FX_SCALE = 18;
		...
		function fxRefractionMap() {
			...
			cv.width = FX_W; cv.height = FX_H;
			const ctx = cv.getContext("2d");
			const img = ctx.createImageData(FX_W, FX_H);
			const d = img.data;
			/* Rounded-rect SDF (negative inside); interior depth = -sd. */
			const sd = (x, y) => {
				const qx = Math.abs(x - (FX_W - 1) / 2) - (FX_W - 1) / 2 + FX_R;
				const qy = Math.abs(y - (FX_H - 1) / 2) - (FX_H - 1) / 2 + FX_R;
				const ax = Math.max(qx, 0), ay = Math.max(qy, 0);
				return Math.min(Math.max(qx, qy), 0) + Math.hypot(ax, ay) - FX_R;
			};
			for (let y = 0; y < FX_H; y += 1) {
			for (let x = 0; x < FX_W; x += 1) {
					let t = 1 + sd(x, y) / FX_RIM;
					t = t < 0 ? 0 : t > 1 ? 1 : t;
					t = t * t * (3 - 2 * t); /* smoothstep */
					/* Outward normal = SDF gradient (numeric probe). */
					const gx = sd(x + 1, y) - sd(x - 1, y);
					const gy = sd(x, y + 1) - sd(x, y - 1);
					const len = Math.hypot(gx, gy) || 1;
					const i = (y * FX_W + x) * 4;
					d[i] = 128 + (gx / len) * 127 * t;
					d[i + 1] = 128 + (gy / len) * 127 * t;
					d[i + 2] = 128;
					d[i + 3] = 255;
				}
			}
		...
		/** Filter markup (tomagranate order): the backdrop is blurred FIRST
		 *  (lightly — liquid glass is clear, not frosted; reference std runs
		 *  0.6–3), then displaced three times at split scales for dispersion,
		 *  recomposed additively and saturated once. Blurring after the
		 *  displacement would smear the refracted rim away. stdDeviation =
		 *  blur-slider / 2 — CSS blur() radii are twice the stdDev. */
		function fxHostMarkup(blurPx) {
			const std = Math.max(0.5, blurPx / 2);
			...
			return `<svg id="${FX_HOST_ID}" width="0" height="0" style="position:absolute;left:0;top:0;pointer-events:none" aria-hidden="true" focusable="false"><defs>` +
				`<filter id="${FX_FILTER_ID}" x="0" y="0" width="100%" height="100%" color-interpolation-filters="sRGB">` +
				`<feImage href="${fxRefractionMap()}" x="0" y="0" width="100%" height="100%" preserveAspectRatio="none" result="map"/>` +
		...
		function syncFxBlur(blurPx) {
			...
			if (g !== null) g.setAttribute("stdDeviation", String(Math.max(0.5, blurPx / 2)));
		}
			...
			return h("div", { className: "ol-field" },
				...
				h("div", { className: "ol-scale", role: "group" },
					presets.map(([id, tv, bv]) =>
						...
						}, `${t("preset" + id[0].toUpperCase() + id.slice(1))} ${tv}%`)),
					h(Pill, {
					active: showSliders,
