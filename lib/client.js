...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DEFAULTS = {
			...
			frostLevel: "balanced", liquidLevel: "balanced",
		};
		const SIDES = ["right", "left"];
		const MATERIALS = ["none", "frost", "liquid"];
		const LAYOUTS = ["compact", "loose"];
		/* One three-step veil preset per glass material — the card reveals the
		   matching field only while that material is selected (KISS: the choice
		   exists exactly when it applies). */
		const LEVELS = ["airy", "balanced", "dense"];
		...
		const COPY = {
			zh: {
				...
				layoutCompact: "紧凑",
				layoutLoose: "宽松",
				frostLevelLabel: "毛玻璃浓度",
				liquidLevelLabel: "液态玻璃浓度",
				lvlAiry: "轻盈",
				lvlBalanced: "均衡",
				lvlDense: "浓郁",
			...
			en: {
				...
				layoutLoose: "Loose",
				frostLevelLabel: "Frost density",
				liquidLevelLabel: "Liquid density",
				lvlAiry: "Airy",
				lvlBalanced: "Balanced",
				lvlDense: "Dense",
		...
		/* ===== glass veil levels ========================================
		   Each glass material exposes one three-step preset on the card
		   (airy | balanced | dense) — a single veil+blur pair per step, so the
		   trade between transparency and legibility is one choice, not a row of
		   sliders. Balanced follows Apple's measured regular material (~72%
		   veil over a small blur — readability comes from the tint veil, not a
		   heavy blur); airy leans translucent, dense leans milk glass. */
		#${NAV_ID}[data-material="frost"] { --ol-veil: 60%; --ol-blur: 14px; }
		#${NAV_ID}[data-material="frost"][data-frost-level="airy"] { --ol-veil: 48%; --ol-blur: 12px; }
		#${NAV_ID}[data-material="frost"][data-frost-level="dense"] { --ol-veil: 72%; --ol-blur: 16px; }
		body[data-ds-dark-theme] #${NAV_ID}[data-material="frost"][data-frost-level="airy"] { --ol-veil: 44%; }
		body[data-ds-dark-theme] #${NAV_ID}[data-material="frost"][data-frost-level="dense"] { --ol-veil: 68%; }
		#${NAV_ID}[data-material="liquid"] { --ol-veil: 65%; --ol-blur: 12px; }
		#${NAV_ID}[data-material="liquid"][data-liquid-level="airy"] { --ol-veil: 52%; --ol-blur: 10px; }
		#${NAV_ID}[data-material="liquid"][data-liquid-level="dense"] { --ol-veil: 74%; --ol-blur: 14px; }
		body[data-ds-dark-theme] #${NAV_ID}[data-material="liquid"][data-liquid-level="airy"] { --ol-veil: 48%; }
		body[data-ds-dark-theme] #${NAV_ID}[data-material="liquid"][data-liquid-level="dense"] { --ol-veil: 70%; }

		/* frost: the veil preset over a saturating blur. */
		#${NAV_ID}[data-material="frost"] .ol-preview {
			background: color-mix(in srgb, var(--dsw-alias-bg-layer-1, #ffffff) var(--ol-veil), transparent);
			-webkit-backdrop-filter: blur(var(--ol-blur)) saturate(1.5);
			backdrop-filter: blur(var(--ol-blur)) saturate(1.5);
		}
		body[data-ds-dark-theme] #${NAV_ID}[data-material="frost"] .ol-preview {
			background: color-mix(in srgb, var(--dsw-alias-bg-layer-1, #232324) var(--ol-veil), transparent);
			-webkit-backdrop-filter: blur(var(--ol-blur)) saturate(1.4);
			backdrop-filter: blur(var(--ol-blur)) saturate(1.4);
			/* No 0 0 1px contact halo in dark: hugging the light ring it
			   rasterizes as a second, parallel dark outline — the hidden double
			   border. Ring + soft drop keep exactly one line. */
		...
		/* liquid: frost's veil with a directional sheen and a whisper of
		   chromatic dispersion at the rim. The veil runs slightly higher than
		...
		#${NAV_ID} {
			--ol-light-x: 26px; --ol-light-deg: 132deg;
			--ol-dis-warm: -.6px -.6px; --ol-dis-cool: .6px .6px;
		}
		#${NAV_ID}[data-side="left"] {
			--ol-light-x: calc(100% - 26px); --ol-light-deg: 228deg;
			--ol-dis-warm: .6px -.6px; --ol-dis-cool: -.6px .6px;
		}
		#${NAV_ID}[data-material="liquid"] .ol-preview {
			background:
				...
				linear-gradient(var(--ol-light-deg),
					...
					transparent 56%),
				color-mix(in srgb, var(--dsw-alias-bg-layer-1, #ffffff) var(--ol-veil), transparent);
			-webkit-backdrop-filter: blur(var(--ol-blur)) saturate(1.6) contrast(1.02);
			backdrop-filter: blur(var(--ol-blur)) saturate(1.6) contrast(1.02);
			/* Rim: the base ring plus two sub-pixel chromatic fringes — warm on
			   the lit corner's edges, cool on the far corner. The ring is listed
			   first (painted on top), so each fringe shows only where its ±0.6px
			   offset pokes past it: dispersion reads as the ring's own colored
			   edge, never a second outline. */
			box-shadow: 0 0 0 1px var(--dsw-alias-border-l2, rgba(0, 0, 0, .1)),
				var(--ol-dis-warm) 0 1px rgba(255, 96, 80, .15),
				var(--ol-dis-cool) 0 1px rgba(96, 160, 255, .15),
				var(--dsw-shadow-lv2, 0 0 1px rgba(0, 0, 0, .2), 0 8px 24px rgba(0, 0, 0, .08));
		}
		body[data-ds-dark-theme] #${NAV_ID}[data-material="liquid"] .ol-preview {
			background:
				...
				linear-gradient(var(--ol-light-deg),
					...
					transparent 56%),
				color-mix(in srgb, var(--dsw-alias-bg-layer-1, #232324) var(--ol-veil), transparent);
			-webkit-backdrop-filter: blur(var(--ol-blur)) saturate(1.5) brightness(.85) contrast(1.05);
			backdrop-filter: blur(var(--ol-blur)) saturate(1.5) brightness(.85) contrast(1.05);
		}
		body[data-ds-dark-theme] #${NAV_ID}[data-material="liquid"] .ol-preview {
			/* One line + dispersion + a soft drop: the old 0 0 1px contact halo
			   is deliberately gone — in dark it read as a hidden second outline. */
			box-shadow: 0 0 0 1px var(--dsw-alias-border-l2, rgba(255, 255, 255, .14)),
				var(--ol-dis-warm) 0 1px rgba(255, 96, 80, .17),
				var(--ol-dis-cool) 0 1px rgba(96, 160, 255, .17),
				0 8px 24px rgba(0, 0, 0, .32);
		}
		...
		function prefsFrom(snap) {
			if (snap !== undefined && snap.status === "ready"
				...
				return {
					...
					frostLevel: LEVELS.includes(v.frostLevel) ? v.frostLevel : DEFAULTS.frostLevel,
					liquidLevel: LEVELS.includes(v.liquidLevel) ? v.liquidLevel : DEFAULTS.liquidLevel,
				};
			}
			return { ...DEFAULTS };
		}
		...
		function OutlineNav() {
			...
			return h("div", {
				...
				"data-frost-level": cfg.frostLevel,
				"data-liquid-level": cfg.liquidLevel,
		...
		function OutlineCard({ copy, scope }) {
			...
			return h("li", { className: "ol-card" + (cardOpen ? " ol-card-open" : "") },
				...
				cardOpen ? h("div", { className: "ol-body" },
					...
					/* Progressive disclosure: a density field exists per glass material
					   and is served only while that material is the active choice — the
					   card stays three fields tall for every material. */
					p.material === "frost" ? h("div", { className: "ol-field" },
						h("span", { className: "ol-label" }, t("frostLevelLabel")),
						h(Scale, {
							value: p.frostLevel,
							options: LEVELS.map((id) => [id, t("lvl" + id[0].toUpperCase() + id.slice(1))]),
							onChange: set("frostLevel"),
						}),
					) : null,
					p.material === "liquid" ? h("div", { className: "ol-field" },
						h("span", { className: "ol-label" }, t("liquidLevelLabel")),
						h(Scale, {
							value: p.liquidLevel,
							options: LEVELS.map((id) => [id, t("lvl" + id[0].toUpperCase() + id.slice(1))]),
							onChange: set("liquidLevel"),
						}),
					) : null,
					h("div", { className: "ol-field" },
