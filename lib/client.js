...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Namespace defaults — mirror of the Host-half schema (lib/index.js);
		   both track the STANDARD preset of each material (the balance point,
		   iOS-regular-anchored — see the preset tables below). */
		const DEFAULTS = {
			side: "right", material: "frost", layout: "compact",
			frostTransparency: 80, frostBlur: 6,
			liquidTransparency: 65, liquidBlur: 6,
		};
		...
		/* [id, transparency, blur] — two crossing axes, not one sliding
		   density dial: 清透/airy maximizes TRANSPARENCY (see-through glass,
		   gentle defocus), 朦胧/misty maximizes BLUR (fog that still glows —
		   the veil stays ≤45% so it can never collapse into the opaque
		   milk-wall the old t30 ladder ended at), 标准/standard is the balance
		   point. It originally anchored on Apple's iOS `regular` material
		   (~t72 / b5.4 / saturate 1.8 — reverse-engineered), which both review
		   rounds scored above every shipped preset; the live user then called
		   that anchor "too conservative" and the 0.0.4 taste round moved it
		   airier to 80/6 — background color temperature shows through while
		   text stays planted. Liquid runs the same semantic ladder but one
		   blur octave lower: its character is the edge lens, and the fold
		   needs comparatively sharp backdrop to bend. */
		const FROST_PRESETS = [["airy", 85, 3], ["standard", 80, 6], ["misty", 55, 18]];
		/* Liquid keeps a blur floor on airy so the card's own text survives
		   high-frequency backdrops in dark theme. Two user rounds walked it
		   85/2 → 80/3 → 75/4: the readable knob is the VEIL, not blur — the
		   subtitle's interference is low-frequency color bands, which blur
		   barely touches (round-5 A/B: +1 blur was imperceptible, +5 veil was
		   the structural fix). Standard then dropped to 65/6 (round-6 final):
		   with airy at 75/4, a 70/4 standard differed by veil alone and the
		   two chips read as one — the ladder needs BOTH axes moving (the
		   frost lesson). Misty sits on the slider lattice (50/10) with a veil
		   clearer than frost's misty — a dark veil with no frost grain to
		   break it read as a flat gray slab (round-3 A/B), while t50 still
		   lets the fog glow through. */
		const LIQUID_PRESETS = [["airy", 75, 4], ["standard", 65, 6], ["misty", 50, 10]];
		...
		/* liquid: frost's veil under a neutral corner sheen and a strict
		   Fresnel rim glint — a small cool-white breath at the lit corner,
		   NOT a warm tint: over the white veil it read as tea-stain
		   yellowing and clashed with dsh's cool dark palette; the far
		   corner keeps a faint cool wash. The veil runs slightly higher than
		...
		#${NAV_ID}[data-material="liquid"] .ol-preview {
			...
			-webkit-backdrop-filter: blur(var(--ol-blur, 12px)) saturate(1.6) contrast(1.02);
			backdrop-filter: blur(var(--ol-blur, 12px)) saturate(1.6) contrast(1.02);
		}
		#${NAV_ID}[data-material="liquid"] .ol-preview::before {
			/* inset -1px: the pseudo box covers the BORDER box (absolute
			   ...
			   and read as a split "double arc" at every corner. Strict
			   one-way Fresnel falloff (peak .30, gone by 45%, no
			   far-corner re-brighten): the old .50 peak with a bounce
			   washed the near border white in light theme and read as
			   a neon filament over dark veils. */
		...
		body[data-ds-dark-theme] #${NAV_ID}[data-material="liquid"] .ol-preview {
			...
			/* No brightness() dimming here — the dark veil token IS the dim (the
			   same one-knob principle as light theme's white veil), and dimming
			   the refracted backdrop would also mute the edge lens we just
			   fixed. A touch of contrast keeps the fold crisp. */
			-webkit-backdrop-filter: blur(var(--ol-blur, 12px)) saturate(1.6) contrast(1.03);
			backdrop-filter: blur(var(--ol-blur, 12px)) saturate(1.6) contrast(1.03);
		}
		body[data-ds-dark-theme] #${NAV_ID}[data-material="liquid"] .ol-preview {
		...
		body[data-ds-dark-theme] #${NAV_ID}[data-material="liquid"][data-refract="1"] .ol-preview {
			-webkit-backdrop-filter: url(#ol-liquid-fx) contrast(1.03);
			backdrop-filter: url(#ol-liquid-fx) contrast(1.03);
		}
		...
		const CORNER_N = 1.5;
		/* Bezel width 16 — the refracting band. FX_R mirrors the element's
		   border-radius (18.4) so the fold hugs the visible edge. 16 sits
		   just inside the corner radius: the band reads as one coherent
		   light-collecting ring (an A/B/C taste ballot — 14/18/22 × 10/18/26
		   — picked 16/18 as “perceptibly bent, never jelly”; 22/26 compressed
		   strokes at the corners and moiré'd grid lines in dark, 14/10 lost
		   the materiality entirely). */
		const FX_RIM = 16;
		/* Base rim displacement (px) — LiquidLens default refraction=18.
		   18 over a 16px band gives a ~7px edge pull: text strokes entering
		   the rim bend 1–2px with structure intact — the “exactly catchable”
		   threshold the ballot validated. */
		const FX_SCALE = 18;
		...
		function fxRefractionMap() {
			...
			const sd = (x, y) => {
				...
				/* The straight/inside branch MUST end in − R (the IQ rounded-box
				   form). A whole release shipped without it: every straight-edge
				   distance was offset +R, so the entire fold band sat R INSIDE the
				   card — the visible edge stayed flat while an invisible warp
				   lived under the text (measured: displacement-map signal only at
				   20–32px depth, neutral 128 through the real rim; corners were
				   exact via the superellipse branch, so the lens was incoherent
				   too). The historical “nested-card annulus seam” was this bug's
				   shadow: the misplaced band's terminus read as an inner card. */
		...
		function OutlineCard({ copy, scope, defaultOpen = false }) {
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				const getCopy = () => copy;
				function LocaleBridge({ subscribe: sub, getCopy: gc, scope: sc, defaultOpen = false }) {
					const c = useSyncExternalStore(sub, gc);
					return h(OutlineCard, { copy: c, scope: sc, defaultOpen });
				}
				const renderConfig = (props, defaultOpen = false) => {
					if (props && props.view === "summary") {
						return copy.description;
					}
					return h(LocaleBridge, { subscribe, getCopy, scope, defaultOpen });
				};

				/* 1. Bundle-level configuration (dsh >= 0.1.6): rendered on the bundle's page in Plugins panel */
				...
				/* 2. Row-level configuration (dsh >= 0.1.6): row gains a Configure control */
				...
				/* 3. Legacy settings tab (dsh <= 0.1.5 backward compatibility):
				 * Gradual sunset plan: to be removed in a future release once all users/environments
				 * are upgraded to dsh 0.1.6+. */
