...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* superellipse(4), not the squircle keyword: in Chrome 152 the
		   keyword parses (CSS.supports: true) but computes to
		   superellipse(2) — a plain circular corner — so the G2
		   upgrade silently never happened. The explicit function is real. */
		...
		#${NAV_ID}[data-material="liquid"] .ol-preview {
			...
			-webkit-backdrop-filter: blur(var(--ol-blur, 12px)) saturate(1.8) contrast(1.02);
		...
		#${NAV_ID}[data-material="liquid"] .ol-preview::before {
			...
			corner-shape: inherit; padding: 1px;
		...
		body[data-ds-dark-theme] #${NAV_ID}[data-material="liquid"] .ol-preview {
			...
			-webkit-backdrop-filter: blur(var(--ol-blur, 12px)) saturate(1.8) brightness(.85) contrast(1.05);
		...
		const FX_W = 300, FX_H = 106, FX_R = 18;
		/* Bezel width 22 — LiquidLens' card value (Panel depth 22, engine
		   default 22): the refracting band, distinct from the 10px G2 radius. */
		const FX_RIM = 22;
		...
		function fxRefractionMap() {
			...
			const d = img.data;
			/* Rounded-rect SDF (negative inside), centered on the pixel grid. */
			const sd = (x, y) => {
		 ...
		 *  displacement, one saturate. Blurring AFTER the displacement would
		 *  smear the refracted rim away; per-channel dispersion was rejected
		 *  (saturated fringes on text edges — see theme-plan). stdDeviation =
		...
		function fxHostMarkup(blurPx) {
			...
			return `<svg id="${FX_HOST_ID}" width="0" height="0" style="position:absolute;left:0;top:0;pointer-events:none" aria-hidden="true" focusable="false"><defs>` +
				...
				`<feColorMatrix in="lens" type="saturate" values="1.8"/>` +
				`</filter></defs></svg>`;
		}
