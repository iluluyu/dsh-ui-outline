...
- **Glass, done carefully.** The per-turn preview card in three finishes: `none` (official opaque tokens), `frost` (default), or `liquid` with a soft light entering from the conversation-facing corner, a warm lit corner and a faint cool far-corner wash. Each glass comes in three densities — airy, balanced, dense — trading transparency for legibility. Corners are G2-continuous where supported (`corner-shape: squircle`, diagonal-matched to the official 10px radius), outlined by a uniform 1px ring. Each finish keeps exactly one rim line; liquid's chromatic touch (warm lit corner, faint cool far corner) lives in the glass body itself, never on the rim. Each glass ships three presets — airy, standard, dense — plus custom transparency and blur-radius sliders. Pure CSS — never overflows the page.
