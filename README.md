...
- **Preview material** — the preview card in three finishes: `frost` (default; 45% token tint + `blur(6px) saturate(1.35)` — the page reads through, text stays readable), `none` (official opaque tokens: bg-layer-1 + border-l2 + shadow-lv2), `liquid` (42% tint + `blur(5px)` with a slightly dimmed backdrop, and a soft light entering from the conversation-facing corner — top-left beside a right rail, top-right beside a left rail). The card's corners are G2-continuous where supported: `corner-shape: squircle` with a diagonally matched radius (18.4px ≈ a 10px G1 arc, Chrome's squircle measured as superellipse(4)); browsers without it fall back to the official 10px G1. Response text uses the readable secondary token rather than caption gray. The resting rail stays bare marks in every mode, like the official rail.
- **预览卡片材质** —— 三种质感：`frost` 毛玻璃（默认；45% token 底色 + `blur(6px)`，背景透出且小字可读）、`none`（官方不透明 token）、`liquid`（液态玻璃：42% 底色 + 5px 轻模糊、背景轻微压暗，光从朝向会话文字的角落柔和射入——右轨左上角、左轨右上角）。卡片圆角在支持的浏览器升级为 G2 连续曲率：`corner-shape: squircle` 半径按对角线等效换算（18.4px ≈ G1 10px 圆弧，实测 Chrome squircle 为 superellipse(4)）；不支持的浏览器回退官方 10px G1。回答小字改用可读性更高的 secondary token。静息轨道始终为官方裸刻度。
